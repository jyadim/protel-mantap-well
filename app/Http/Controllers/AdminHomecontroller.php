...
class AdminHomeController extends Controller
{
    public function index()
    {
        ...
        $displayedProductIds = HomeProduct::pluck('name')->toArray();

        // Fetch products that are not displayed on the home page
        $availableProducts = Products::whereNotIn('title', $displayedProductIds)->get();
        return view('dashboardAdmin', compact('banner', 'prodhome', 'quickaccess', 'news', 'users', 'product', 'ref', 'availableProducts'));
    }
    ...
    public function prdcreate(Request $request)
    {
        DB::beginTransaction();
        ...
        try {
            ...
            // Create a new HomeProduct instance
            $product = HomeProduct::create([
                'name' => $validated['name'],
                'image_path' => $fileName,
                'image_name' => $validated['name'],
                'slug' => $newSlug,
                'user_id' => $validated['author_name'],
            ]);
            Log::info('HomeProduct created', ['product' => $product]);
    
            // Update the detail_products table with the new home_slug
            DetailProducts::where('home_slug', null)->update(['home_slug' => $newSlug]);
            Log::info('DetailProducts updated', ['home_slug' => $newSlug]);
            ...
            Session::flash('error', 'An error occurred while adding the product. ' . $e->getMessage());
            return redirect()->route('admin.dashboard');
        }
    }
