    ...
    <style>
       .input-wrapper {
        ...
        .input-field:not(:placeholder-shown) + .input-label {
            ...
            left: 0.75rem;
            font-size: 0.75rem;
            color: #34d399; /* Tailwind lime-400 color */
        }
        ...
        .input-label {
            ...
            font-size: 1rem;
            color: #6b7280; /* Tailwind neutral-500 color */
            pointer-events: none;
            transition: all 0.2s ease-out;
        }
        ...
        .submit-button {
            ...
            background-color: #84cc16; /* Tailwind lime-500 color */
            color: white;
            font-size: 0.875rem;
            font-weight: 500;
        ...
        .submit-button:hover {
            background-color: #65a30d; /* Tailwind lime-600 color */
        }
    </style>
        ...
        <section class="mb-16">
            <div>
                <div id="map" class="mt-16 relative h-[300px] overflow-hidden bg-cover bg-[50%] bg-no-repeat">
                    <iframe src="{{ $contact_info->link_maps }}" width="100%" height="480" style="border:0;"
                ...
                <div class="container px-6 md:px-12 ">
                    <div
                        ...
                        <div class="flex flex-wrap">
                            ...
                            @endif
                            <form action="{{ route('contact.send') }}" method="POST" class="max-w-lg mx-auto bg-white p-6 rounded shadow-md">
                                @csrf
                                @method('POST')
                        
                                <div class="input-wrapper">
                                    <input type="text"
                                        class="input-field peer block min-h-[auto] rounded border-2 border-gray-300 bg-transparent py-2 px-3 leading-6 outline-none transition-all duration-200 ease-linear"
                                ...
                                <div class="input-wrapper">
                                    ...
                                    <label for="phone" class="input-label">Phone Numbers</label>
                                </div>
                        
                                <div class="input-wrapper">
                                    <textarea
                                        class="input-field peer block min-h-[auto] rounded border-2 border-gray-300 bg-transparent py-2 px-3 leading-6 outline-none transition-all duration-200 ease-linear"
                                        id="address" name="address" rows="3" placeholder=" "></textarea>
                                ...
                                <div class="input-wrapper">
                                    <textarea
                                        class="input-field peer block min-h-[auto] rounded border-2 border-gray-300 bg-transparent py-2 px-3 leading-6 outline-none transition-all duration-200 ease-linear"
                                        id="message" name="message" rows="3" placeholder=" "></textarea>
                                    <label for="message" class="input-label">Message</label>
                                </div>
                        
                                <div class="input-wrapper numberVerify">
                                    <input type="text"
                                        class="input-field peer block min-h-[auto] rounded border-2 border-gray-300 bg-transparent py-2 px-3 leading-6 outline-none transition-all duration-200 ease-linear"
                                        id="verify" name="verify" placeholder=" " />
                                    <label for="verify" class="input-label">
                                        Input The Following Numbers: <span id="verifyNum"></span>
                                    </label>
                                </div>
                        
                                <button type="submit"
                                    class="w-full rounded bg-lime-500 text-white px-6 py-2 text-xs font-medium uppercase leading-normal hover:bg-lime-600">
                                    Contact Us
                                </button>
                            </form> 
                            </div>
...
<script>
    ...
    // Set the random number to the span with id "verifyNum"
    document.getElementById('verifyNum').textContent = randomNumber;
    ...
    function verifyNumber() {
        ...
        if (parseInt(userInput) === randomNumber) {
            Swal.fire({
                icon: 'success',
                title: 'Correct!',
                text: 'The number matches!',
            });
        } else {
            Swal.fire({
                icon: 'error',
                title: 'Incorrect!',
                text: 'The number does not match!',
            });
        }
    }
</script>
